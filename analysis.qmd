...
```{python}
#| echo: false
# Load complaint gap areas
complaint_gap = gpd.read_file('geojson/complaintgap/complaint_gap_areas.geojson')
...
# Add complaint gap areas in red
folium.GeoJson(
    complaint_gap.to_json(),
    style_function=lambda x: {
        'fillColor': '#FF6B6B',
        'color': '#FF6B6B',
        'weight': 2,
    ...
    popup=folium.GeoJsonPopup(fields=['GEOID', 'median_inc', 'viirs_mean', 'streetli_5'],
                                aliases=['Block Group', 'Median Income', 'VIIRS Brightness', 'Complaints/km²']),
    name='Complaint Gap Areas'
).add_to(m6)
...
```{python}
#| echo: false
# Load darkening trend areas
darkening = gpd.read_file('geojson/darkinging_trend_areas/darkening_trend_areas.geojson')
...
folium.GeoJson(
    ...
    name='Predicted Darkening Areas'
).add_to(m7)
...
<div class="stat-card stat-card-blue">
<span class="stat-number">R² = 0.997</span>
...
```{python}
#| echo: false
# Load safe paths and route locations
safe_paths = gpd.read_file('geojson/safepath/safepath.geojson')
route_locations = gpd.read_file('geojson/routelocation/route_location.geojson')
...
# Add route locations (origins/destinations)
for idx, row in route_locations.iterrows():
    folium.Marker(
        location=[row.geometry.y, row.geometry.x],
        popup=row['location_name'] if 'location_name' in row else f"Location {idx}",
